...
if [[ -z "$MEMBER_ID" ]]; then
  blocked_case "$CASE_ID" "AUTO_SH" "$(runtime_value_path current_member_id)" "Missing MEMBER_ID. Run 05-create-child.sh first or export MEMBER_ID."
fi

ARTIFACT="$ARTIFACT_DIR/${CASE_ID}-generate-schedule.json"
STATUS="$(curl -sS -b "$COOKIE_JAR" -X POST "$BASE_URL/api/family-members/$MEMBER_ID/schedule/from-template" -o "$ARTIFACT" -w "%{http_code}" || true)"
